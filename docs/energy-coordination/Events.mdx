...
The events endpoints can be used by partners to fetch events from the Energy Coordination API in case they were not able to receive the event through the webhooks (for example when starting the client server).
