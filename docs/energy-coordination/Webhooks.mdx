...
When implementing your webhook endpoint, consider the following rules:

- **Always return a `200 OK` response**: This is important to acknowledge receipt of the event. If you need to perform long processing, save the event for later processing or enqueue it and return a `200 OK` response immediately.
